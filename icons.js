'use strict';
/* Icon set: simple 24px line icons drawn with currentColor, so they follow text colour and theme.
   Use ic('name', size) in strings, or <i data-icon="name"></i> in HTML (hydrated at startup). */
const ICON_PATHS = {
  mark: '<path d="M12 3.5a8.5 8.5 0 1 0 8.5 8.5"/><circle cx="12" cy="12" r="2.6" fill="currentColor" stroke="none"/><circle cx="18.4" cy="5.6" r="1.4" fill="currentColor" stroke="none"/>',
  ...
  memory: '<rect x="2" y="7" width="20" height="10" rx="1.5"/><path d="M6 11v2M10 11v2M14 11v2M18 11v2M6 17v3M18 17v3"/>',
  ...
  'globe-off': '<path d="M1 1l22 22"/><path d="M16.72 11.06A10.94 10.94 0 0 1 19 12.55"/><path d="M5 12.55a10.94 10.94 0 0 1 5.17-2.39"/><path d="M10.71 5.05A16 16 0 0 1 22.58 9"/><path d="M1.42 9a15.91 15.91 0 0 1 4.7-2.88"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><circle cx="12" cy="20" r="1" fill="currentColor" stroke="none"/>',
};

function ic(name, size = 16, cls = '') {
  const p = ICON_PATHS[name] || ICON_PATHS.info;
  return `<svg class="ic${cls ? ' ' + cls : ''}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${p}</svg>`;
}
function hydrateIcons(root = document) {
  root.querySelectorAll('i[data-icon]').forEach((e) => { e.outerHTML = ic(e.dataset.icon, +e.dataset.size || 16); });
}
hydrateIcons();
